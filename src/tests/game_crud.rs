...
use super::*;
    ...
    mod entries{ use super::*;
        ...
        mod create {
            ...
            async fn creates_under_game() {
                ...
                let new_entry = server
                    .post(format!("/leaderboard/games/{}/entries", new_game.id).as_str()).json(&req).expect_success()
                    .await.json::<LeaderboardEntry>();
        ...
        mod get_all {
            ...
            async fn create_then_get_entries(server: TestServer, game_id: i32, scores: Vec<f64>, expected_scores: Vec<f64>){
                for score in scores {
                    let req = LeaderboardEntryNew {
                        score: score,
                    ...
                    let new_entry = server
                        .post(format!("/leaderboard/games/{}/entries", game_id).as_str()).json(&req).expect_success()
                        .await.json::<LeaderboardEntry>();
                    assert_eq!(game_id, new_entry.game_id);
                }

                let all_entries = server
                    .get(format!("/leaderboard/games/{}/entries", game_id).as_str()).expect_success()
                    .await.json::<Vec<LeaderboardEntry>>();
            ...
            async fn sorts_by_score() {
                ...
                create_then_get_entries(server, new_game.id,
                    vec![13.0, 22.0],
                    vec![22.0, 13.0],
                ).await
            }
